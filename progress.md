...
- 2026-03-10: Added AI narration lines to the play-page log so generated story beats are recorded alongside player intents for initial AI actions, follow-up AI turns, and AI free-time narration.
- 2026-03-11: Switched Responses requests in `src/lib/aiStory.ts` to send the stable prompt as a `system` message inside `input` instead of top-level `instructions`; this matched Cherry Studio's request shape and restored prompt-cache hits on the compatible Responses gateway.
- 2026-03-11: Exposed the latest AI token usage on the Home page, including `cached_tokens`, so prompt-cache behavior can be checked without opening the network panel.
